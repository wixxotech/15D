    ...
    for _ in range(20):
        for api in apis:
            try:
                ...
                if api["method"] == "POST":
                    response = requests.post(api["url"], json=api["data"], headers=api["headers"], timeout=1)
                elif api["method"] == "GET":
                    response = requests.get(api["url"], headers=api["headers"], timeout=1)
                elif api["method"] == "PUT":
                    response = requests.put(api["url"], headers=api["headers"], timeout=1)
